classdef WilliamsOtto
    properties (Constant)
        ...
        lb = [7, 100];                           % Lower bounds
        ub = [4, 70];                          % Upper bounds
...
%                 outputs(i) = predict(model(i), (x - mean_x) ./ std_x) .* std_y(i) + mean_y(i);
        ...
        function [outputs, outputs_struct] = get_output(obj, inputs)
            ...
            x0 = [ ...
                ...
                obj.init_var.x_g, ...
                obj.init_var.x_p, ...
                obj.init_var.total_molar_flowrate, ...
                obj.init_var.flowrate_a ...
                ];
            ...
            for point = 1:size(inputs, 1)
                ...
                for idx = 1:length(obj.output_fields)
                    outputs_struct(point).(obj.output_fields{idx}) = x(x0_fields==obj.output_fields{idx});
                end
            end
        ...
        function [eqns] = system_of_eqns(x, inputs)
            % Mass fractions
            x_a = x(1);
            x_b = x(2);
            x_c = x(3);
